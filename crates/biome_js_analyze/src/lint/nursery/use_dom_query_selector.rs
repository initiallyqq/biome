...
impl Rule for UseDomQuerySelector {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        if is_ignored_receiver(&object, ctx.options()) || is_definitely_not_dom_node(&object) {
            return None;
        }
...
fn is_ignored_receiver(expr: &AnyJsExpression, options: &UseDomQuerySelectorOptions) -> bool {
